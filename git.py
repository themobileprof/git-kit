...
class MainMenu:
    def __init__(self):
        ...
        # 1a. Installation
        if not misc.which("git"):
            self.git_class.git_inst()

        # 1b. Ensure you're in git directory 
        if not self.git_class.is_git():
            # Exit if not git init
            if not self.git_class.git_init():
                exit()

        # 1c. Configuration
        if not self.git_class.get_git_conf("user.name"):
