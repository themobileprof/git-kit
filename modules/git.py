...
class Git:
    ...
    # Configure git
    def get_git_conf(self, entry):
        time.sleep(1)
        config=subprocess.run(['git', 'config', entry], capture_output=True, text=True)
        return True if config.returncode == 0 and config.stdout.strip() != "" else False
